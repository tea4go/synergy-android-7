 ...
 *
 *  Modifier to allow overriding some properties
 */
class Timber private constructor() {
    ...
    abstract class Tree {
        ...
        open fun verbose(t: Throwable?, message: String?, vararg args: Any?) {
            prepareLog(Log.VERBOSE, t, message, *args)
        }

        /** Log a verbose exception. */
        open fun verbose(t: Throwable?) {
        ...
        open fun debug(t: Throwable?, message: String?, vararg args: Any?) {
            prepareLog(Log.DEBUG, t, message, *args)
        }

        /** Log a debug exception. */
        open fun debug(t: Throwable?) {
        ...
        open fun info(t: Throwable?, message: String?, vararg args: Any?) {
            prepareLog(Log.INFO, t, message, *args)
        }

        /** Log an info exception. */
        open fun info(t: Throwable?) {
        ...
        open fun warn(t: Throwable?, message: String?, vararg args: Any?) {
            prepareLog(Log.WARN, t, message, *args)
        }

        /** Log a warning exception. */
        open fun warn(t: Throwable?) {
        ...
        open fun error(t: Throwable?, message: String?, vararg args: Any?) {
    ...
    companion object Forest : Tree() {
        /** Log a verbose message with optional format args. */
        @JvmStatic
        override fun verbose(@NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.verbose(message, *args) }
        }

        /** Log a verbose exception and a message with optional format args. */
        @JvmStatic
        override fun verbose(t: Throwable?, @NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.verbose(t, message, *args) }
        }

        /** Log a verbose exception. */
        @JvmStatic
        override fun verbose(t: Throwable?) {
            treeArray.forEach { it.verbose(t) }
        }

        /** Log a debug message with optional format args. */
        @JvmStatic
        override fun debug(@NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.debug(message, *args) }
        }

        /** Log a debug exception and a message with optional format args. */
        @JvmStatic
        override fun debug(t: Throwable?, @NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.debug(t, message, *args) }
        }

        /** Log a debug exception. */
        @JvmStatic
        override fun debug(t: Throwable?) {
            treeArray.forEach { it.debug(t) }
        }

        /** Log an info message with optional format args. */
        @JvmStatic
        override fun info(@NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.info(message, *args) }
        }

        /** Log an info exception and a message with optional format args. */
        @JvmStatic
        override fun info(t: Throwable?, @NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.info(t, message, *args) }
        }

        /** Log an info exception. */
        @JvmStatic
        override fun info(t: Throwable?) {
            treeArray.forEach { it.info(t) }
        }

        /** Log a warning message with optional format args. */
        @JvmStatic
        override fun warn(@NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.warn(message, *args) }
        }

        /** Log a warning exception and a message with optional format args. */
        @JvmStatic
        override fun warn(t: Throwable?, @NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.warn(t, message, *args) }
        }

        /** Log a warning exception. */
        @JvmStatic
        override fun warn(t: Throwable?) {
            treeArray.forEach { it.warn(t) }
        }

        /** Log an error message with optional format args. */
        @JvmStatic
        override fun error(@NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.error(message, *args) }
        }

        /** Log an error exception and a message with optional format args. */
        @JvmStatic
        override fun error(t: Throwable?, @NonNls message: String?, vararg args: Any?) {
            treeArray.forEach { it.error(t, message, *args) }
        }

        /** Log an error exception. */
        @JvmStatic
        override fun error(t: Throwable?) {
            treeArray.forEach { it.error(t) }
        }
